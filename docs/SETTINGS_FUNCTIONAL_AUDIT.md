...
| Recording display       | Setting | `ThemePackSelector` -> filtered Classic/Pirate Scribe catalog -> apply command -> theme-pack manager -> active pack/presets -> overlay renderer          | **Fixed/tested.** The production picker intentionally omits third-party install/remove actions and migrates retired selections to Classic.                                               |
...
| Post-processing enabled           | Setting plus pipeline policy  | toggle -> command -> shortcut/sidebar/footer gates plus ordinary `transcribe` routing -> selected provider/profile -> safe raw-text fallback | **Fixed/tested; provider behavior manual.** Ordinary dictation uses the selected cleanup profile only while enabled; the dedicated shortcut remains an explicit override. Fresh installs default off, while presets such as Pirate Scribe can opt in.              |
...
- Exercise Classic and Pirate Scribe displays, including style, position, and animation choices. Windows themed-widget dragging and lifecycle retention were manually verified; macOS panels and Linux layer-shell themes intentionally remain anchored in v1.
- Download/import/select representative models and exercise GPU/ORT accelerators on compatible hardware.
- Exercise post-process providers with real credentials/endpoints, Codex CLI availability, profile CRUD, and a provider-backed ordinary dictation transformation in the running app. Routing, success replacement, and raw-text fallback have deterministic backend coverage.
